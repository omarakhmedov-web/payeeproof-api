...
from payeeproof_api.monerium_config import (
    ...
    monerium_chain_variants as monerium_chain_variants_impl,
    monerium_effective_source_chain as monerium_effective_source_chain_impl,
    monerium_is_sandbox_env as monerium_is_sandbox_env_impl,
    normalize_monerium_chain as normalize_monerium_chain_impl,
)
from payeeproof_api.monerium_helpers import (
    build_monerium_authorize_url,
    monerium_build_counterpart_from_payload as monerium_build_counterpart_from_payload_impl,
    monerium_build_order_submit_gate as monerium_build_order_submit_gate_impl,
    monerium_counterpart_details_complete as monerium_counterpart_details_complete_impl,
    monerium_order_phase as monerium_order_phase_impl,
    monerium_order_phase_label as monerium_order_phase_label_impl,
    monerium_order_state_text as monerium_order_state_text_impl,
    monerium_order_summary as monerium_order_summary_impl,
    monerium_pkce_challenge,
    monerium_pkce_verifier,
    monerium_place_order_message_hint as monerium_place_order_message_hint_impl,
    monerium_response_page as monerium_response_page_impl,
    monerium_token_symbol as monerium_token_symbol_impl,
    normalize_monerium_return_to as normalize_monerium_return_to_impl,
    parse_bool_flag,
)

APP_VERSION = "2.6.2-monerium-arbitrum-accountfix"
...
def normalize_monerium_chain(value: Any) -> str:
    return normalize_monerium_chain_impl(
        value,
        default_chain=MONERIUM_DEFAULT_CHAIN,
        allowed_chains=MONERIUM_ALLOWED_CHAINS,
        aliases=MONERIUM_REGISTRY_ALIASES,
    )


def monerium_is_sandbox_env() -> bool:
    return monerium_is_sandbox_env_impl(MONERIUM_API_BASE)


def monerium_chain_variants(chain: Any) -> List[str]:
    return monerium_chain_variants_impl(
        chain,
        api_base=MONERIUM_API_BASE,
        default_chain=MONERIUM_DEFAULT_CHAIN,
        allowed_chains=MONERIUM_ALLOWED_CHAINS,
        aliases=MONERIUM_REGISTRY_ALIASES,
    )
...
def monerium_effective_source_chain(source_address_record: Dict[str, Any], requested_chain: Any) -> str:
    return monerium_effective_source_chain_impl(
        source_address_record,
        requested_chain,
        api_base=MONERIUM_API_BASE,
        default_chain=MONERIUM_DEFAULT_CHAIN,
        allowed_chains=MONERIUM_ALLOWED_CHAINS,
...
def monerium_authorize_url(*, state: str, code_challenge: str, chain: str, skip_kyc: bool = False) -> str:
    return build_monerium_authorize_url(
        state=state,
        ...
        api_base=MONERIUM_API_BASE,
        include_response_type=MONERIUM_INCLUDE_RESPONSE_TYPE,
        include_chain_in_auth_url=MONERIUM_INCLUDE_CHAIN_IN_AUTH_URL,
        normalize_chain=normalize_monerium_chain,
        append_url_query=append_url_query,
    )


def normalize_monerium_return_to(value: Any) -> str:
    return normalize_monerium_return_to_impl(
        value,
        public_demo_hosts=PUBLIC_DEMO_HOSTS,
        public_demo_origins=PUBLIC_DEMO_ORIGINS,
        allowed_origins=allowed_origins,
        split_origin_hosts=_split_origin_hosts,
    )
...
def monerium_response_page(title: str, message: str, *, status_code: int = 200, details: Optional[Dict[str, Any]] = None) -> Tuple[str, int, Dict[str, str]]:
    return monerium_response_page_impl(title, message, status_code=status_code, details=details)
...
def upsert_nowpayments_order_from_ipn(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        existing_order_id = normalize_text((row_to_dict(existing).get("order_id") if existing else ""), 120)
        resolved_order_id = order_id or existing_order_id or purchase_id or nowpayments_checkout_id()
...
def monerium_order_state_text(order: Dict[str, Any]) -> str:
    return monerium_order_state_text_impl(order)


def monerium_order_phase(order: Dict[str, Any]) -> str:
    return monerium_order_phase_impl(order)


def monerium_order_phase_label(phase: str) -> str:
    return monerium_order_phase_label_impl(phase)


def monerium_order_summary(order: Dict[str, Any]) -> Dict[str, Any]:
    return monerium_order_summary_impl(
        order,
        normalize_text=normalize_text,
        normalize_money_amount=normalize_money_amount,
        normalize_chain=normalize_monerium_chain,
...
def monerium_token_symbol(currency: Any) -> str:
    return monerium_token_symbol_impl(currency)
...
def monerium_select_account(profile_payload: Dict[str, Any], *, account_id: str = "", chain: str = "", currency: str = "eur") -> Dict[str, Any]:
    ...
    requested_variants = set(monerium_chain_variants(normalized_chain))
    ...
    for account in accounts:
        ...
        account_currency = str(account.get("currency") or "").strip().lower()
        if account_currency != normalized_currency:
            continue
        chain_match = (not requested_variants) or (account_chain in requested_variants) or (not account_chain)
        if chain_match:
            return dict(account)
        if not selected and account_chain == normalized_chain:
...
def monerium_place_order_message_hint(amount: str, iban: str) -> str:
    return monerium_place_order_message_hint_impl(
        amount,
        iban,
        normalize_money_amount=normalize_money_amount,
        normalize_iban=normalize_iban,
...
def monerium_build_counterpart_from_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    return monerium_build_counterpart_from_payload_impl(payload, normalize_iban=normalize_iban)


def monerium_counterpart_details_complete(counterpart: Dict[str, Any]) -> bool:
    return monerium_counterpart_details_complete_impl(counterpart)
...
def monerium_build_order_submit_gate(*, has_source: bool, iban_ok: bool, counterpart_ready: bool, balance_known: bool, balance_ok: bool) -> Dict[str, Any]:
    return monerium_build_order_submit_gate_impl(
